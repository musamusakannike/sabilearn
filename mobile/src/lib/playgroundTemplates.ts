import { PlaygroundKind } from './types';
...
export function defaultFiles(kind: PlaygroundKind): Record<string, string> {
  if (kind === 'python') {
    return { 'main.py': STARTERS['main.py'] };
  }
  return {
...
export function fileLanguage(file: string): 'python' | 'css' | 'javascript' | 'xml' {
